# This script was provided by utaha.
...
class MtCracker:
	...
	def update(self, pos: int, bit_pos: int, value: bool) -> None:
		...
		"""
		while self.cache is None or self.cache[0] < pos:
			idx = 0 if self.cache is None else self.cache[0] + 1
			self.cache = (idx, self.rand())

		self.solver.add(self.cache[1][bit_pos], value)
