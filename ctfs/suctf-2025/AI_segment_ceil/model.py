...
class DDPM(nn.Module):
    def __init__(self):
        ...
        self.in_chs = [32, 32, 96, 128]
        self.num_resolutions = len(self.in_chs)
        self.all_resolutions = all_resolutions = [512 // (2 ** i) for i in range(self.num_resolutions)]
        ...
        modules.append(ddpm_conv3x3(3, self.in_chs[0]))

        for i in range(self.num_resolutions-1):
            in_ch = self.in_chs[i]
            out_ch = self.in_chs[i+1]
            for j in range(self.num_res_blocks):
                hs_c.append(in_ch)
                modules.append(ResnetBlockDDPM(nn.SiLU(), temb_dim=None, dropout=0.05, in_ch=in_ch, out_ch=out_ch))
                in_ch = out_ch
            hs_c.append(in_ch)
            modules.append(Downsample(in_ch, with_conv=True))
        
        in_ch = self.in_chs[-1]
        modules.append(ResnetBlockDDPM(nn.SiLU(), temb_dim=None, dropout=0.05, in_ch=in_ch, out_ch=in_ch))
        modules.append(AttnBlock(in_ch))
        modules.append(ResnetBlockDDPM(nn.SiLU(), temb_dim=None, dropout=0.05, in_ch=in_ch, out_ch=in_ch))

        for i in reversed(range(self.num_resolutions-1)):
            in_ch = self.in_chs[i+1]
            out_ch = self.in_chs[i]
            modules.append(Upsample(in_ch, with_conv=True))
            for j in range(self.num_res_blocks+1):
                out_ch = self.in_chs[i] if j == self.num_res_blocks else in_ch
                modules.append(ResnetBlockDDPM(nn.SiLU(), temb_dim=None, dropout=0.05, in_ch=in_ch + hs_c.pop(), out_ch=out_ch))
        
        modules.append(nn.GroupNorm(num_groups=32, num_channels=self.in_chs[0], eps=1e-6))
        modules.append(ddpm_conv3x3(self.in_chs[0], 1))
            
        self.all_modules = nn.ModuleList(modules)


    def forward(self, x):
        assert x.min() >= 0.0 and x.max() <= 1.0, 'input should be normalized'

        module_idx = 0
        h_stacks = []

        x = self.all_modules[module_idx](x)
        module_idx += 1
        ...
        for i in reversed(range(self.num_resolutions-1)):
            ...
            module_idx += 1
            for j in range(self.num_res_blocks+1):
                x = self.all_modules[module_idx](torch.cat([x, h_stacks.pop()], dim=1))
                module_idx += 1
        
        x = self.all_modules[module_idx](x) # GroupNorm
        module_idx += 1
        x = self.all_modules[module_idx](x) # Conv3x3
        module_idx += 1

        assert module_idx == len(self.all_modules)
