# https://github.com/mjosaarinen/uov-py
...
class UOV:
    ...
    def unpack_mtri(self, b, d):
        ...
        for i in range(d):
            for j in range(i, d):
                m[i][j] = int.from_bytes( b[ p : p + self.m_sz ] , 'big')
                p += self.m_sz
        return m
    ...
    def pack_mrect(self, m, h, w):
        ...
        for i in range(h):
            for j in range(w):
                b += m[i][j].to_bytes(self.m_sz, 'big')
        return b
    ...
    def gauss_solve(self, l, c):
        ...
        h   =   self.m
        w   =   self.m + 1
        l   =   [ self.gf_unpack( x.to_bytes(self.m_sz, 'big') ) for x in l ]
    ...
    def pubmap(self, z, tm):
        ...
        return y.to_bytes(self.m_sz, 'big')
    ...
    def sign(self, msg, sk):
        ...
        while x == None and ctr < 0x100:
            ...
            r = self.gf_unpack(r.to_bytes(self.m_sz, 'big'))
